...
$GLOBALS['balises'] = array(
	...
	'art_encart' => '{articles_encart}',
	'links_encart' => '{links_encart}',
...
function conversions_theme($texte, $solo_art, $cnt_mode) {
	...
	$texte = str_replace($GLOBALS['balises']['links_encart'], encart_links(), $texte);
	$texte = str_replace($GLOBALS['balises']['art_encart'], encart_articles(), $texte);
...
// Tentative affichage about.html
function conversions_theme_static($texte, $billet) {
...
function afficher_index($tableau, $type) {
	...
	elseif ($type == 'post') {
		// Vérifie si l'on a affaire à une page statique ou une liste d'articles
		$tmp = false;
		foreach ($GLOBALS['static_tags']['articles'] as $key => $value) {
			$val = strtolower($value);
			if (isset($_GET[strtolower($value)])){
				$tmp = true;
			}
		}
		// traitement des pages statiques
		if ($tmp) {

			$billet = $tableau;

			// replace {boucle commentaires} in $theme_post
			$theme_post = file_get_contents($GLOBALS['theme_post_static']);
			$v = extract_boucles($theme_post, '', 'incl');
			// var_dump($v);

			$HTML_article = conversions_theme_static($theme_post, $billet);
			$HTML = str_replace($v, '', $HTML_article);
			...
			// var_dump($GLOBALS['boucles']['posts']);
			// replace tags.
